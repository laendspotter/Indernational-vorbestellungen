...
module.exports = async (req, res) => {
  ...
  const dayBars = ['Mo','Di','Mi','Do','Fr'].map(d => {
    ...
    return `<tr>
      ...
      <td style="padding:4px 0;">
        <div style="background:#1e2236;border-radius:4px;overflow:hidden;height:14px;width:180px;">
          <div style="background:linear-gradient(90deg,#f5a623,#3ecf8e);height:100%;width:${pct}%;border-radius:4px;"></div>
        </div>
      </td>
  ...
  const topKundenRows = topKunden.map(([name, count], i) => {
    const medals = ['🥇','🥈','🥉','4.','5.'];
    return `<tr>
      <td style="padding:8px 12px 8px 0;font-size:0.85rem;">${medals[i]}</td>
      <td style="padding:8px 0;font-size:0.85rem;color:#f0ede6;">${name}</td>
      <td style="padding:8px 0;font-size:0.85rem;color:#f5a623;text-align:right;">${count}x</td>
    </tr>`;
  }).join('');
...
<div style="max-width:600px;margin:0 auto;padding:40px 20px;">
  ...
  <h1 style="font-size:1.6rem;font-weight:900;margin:0 0 4px;letter-spacing:-0.02em;">Letzte Woche</h1>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
    <tr>
      <td style="width:25%;text-align:center;background:#131726;border:1px solid #1e2236;border-radius:10px;padding:16px;">
        ...
        <div style="font-size:0.7rem;color:#7a7468;text-transform:uppercase;letter-spacing:0.06em;margin-top:3px;">Gesamt</div>
      </td>
      <td style="width:4%;"></td>
      <td style="width:25%;text-align:center;background:#131726;border:1px solid #1e2236;border-radius:10px;padding:16px;">
        <div style="font-size:1.8rem;font-weight:900;color:#f5a623;">${truck.length}</div>
        <div style="font-size:0.7rem;color:#7a7468;text-transform:uppercase;letter-spacing:0.06em;margin-top:3px;">🚚 Truck</div>
      </td>
      <td style="width:4%;"></td>
      <td style="width:25%;text-align:center;background:#131726;border:1px solid #1e2236;border-radius:10px;padding:16px;">
        <div style="font-size:1.8rem;font-weight:900;color:#3ecf8e;">${kantine.length}</div>
        <div style="font-size:0.7rem;color:#7a7468;text-transform:uppercase;letter-spacing:0.06em;margin-top:3px;">🍽️ Kantine</div>
      </td>
      <td style="width:4%;"></td>
      <td style="width:25%;text-align:center;background:#131726;border:1px solid #1e2236;border-radius:10px;padding:16px;">
        <div style="font-size:1.8rem;font-weight:900;color:#f5a623;">${revenue.toFixed(2).replace('.',',')} €</div>
        <div style="font-size:0.7rem;color:#7a7468;text-transform:uppercase;letter-spacing:0.06em;margin-top:3px;">Umsatz</div>
      </td>
    </tr>
  </table>
  ...
  <div style="background:#131726;border:1px solid #1e2236;border-radius:12px;padding:20px;margin-bottom:20px;">
    ...
    <div style="display:flex;gap:16px;">
      <div style="flex:1;text-align:center;background:#0b0e1a;border-radius:8px;padding:12px;">
        <div style="font-size:1.4rem;font-weight:900;color:#f5a623;">${m1}</div>
        <div style="font-size:0.75rem;color:#7a7468;margin-top:2px;">Menü 1</div>
      </div>
      <div style="flex:1;text-align:center;background:#0b0e1a;border-radius:8px;padding:12px;">
        <div style="font-size:1.4rem;font-weight:900;color:#3ecf8e;">${m2}</div>
        <div style="font-size:0.75rem;color:#7a7468;margin-top:2px;">Menü 2</div>
      </div>
    </div>
  </div>
  ...
  <!-- TOP KUNDEN -->
  ${topKunden.length > 0 ? `
  <div style="background:#131726;border:1px solid #1e2236;border-radius:12px;padding:20px;margin-bottom:28px;">
    <p style="font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:#7a7468;margin:0 0 14px;">Top Kunden</p>
    <table width="100%" cellpadding="0" cellspacing="0">${topKundenRows}</table>
  </div>` : ''}
